        ...
        <v-container fluid>
            <v-window v-model="localActiveItem">
                ...
                <v-window-item value="order">
                    ...
                    <v-btn v-if="isMobile" block color="red" @click="$emit('logout')" class="mt-4">Logout</v-btn>
                </v-window-item>
            ...
            <v-card v-if="orderDetailModal.data" ref="orderDetailCard">
                ...
                <v-card-text>
                    <v-list lines="two">
                        ...
                        <v-list-item title="Angsuran">
                            <template v-slot:subtitle>
                                <div>Rp {{ (orderDetailModal.data.installmentPrice || 0).toLocaleString('id-ID') }} / {{ orderDetailModal.data.paymentFrequency }}</div>
                                <div>Tenor: {{ orderDetailModal.data.tenor }} hari</div>
                            </template>
                        </v-list-item>
...
const KolektorDashboard = {
    ...
    setup(props, { emit }) {
        ...
        const filteredBills = computed(() => myActiveBills.value.filter(b => {
            if (b.paymentFrequency === 'mingguan') {
                const startDate = b.timestamp?.toDate();
                if (!startDate) return false;
                const weeksPassed = Math.floor(((new Date() - startDate) / (1000 * 60 * 60 * 24 * 7)));
                const paymentsMade = Math.floor((b.payments || []).length / 6);
                return weeksPassed >= paymentsMade;
            }
            return true;
        }).filter(b => b.consumerName.toLowerCase().includes(searchTerm.value.toLowerCase())));
...
createApp({
    ...
    setup() {
        ...
        return { user, loading, authLoading, profileLoading, authError, snackbar, appData, isProfileComplete, isMobile: mobile, activeItem, currentDashboardComponent, currentNavItems, navColor, navMenus, drawer, handleLogin, handleRegister, handleLogout, handleSaveProfile, handleAction };
    }
